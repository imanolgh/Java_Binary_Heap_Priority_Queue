...
/**
 * These tests focus on the MinPriorityQueue Interface methods behavior
 * (They are about a PQ's behavior and not the specific details of a healp implementation)
 */
...
class BasicMinPQTests {
	...
	@Order(10) 
	@Timeout(value = 500, unit = TimeUnit.MILLISECONDS)
	public void testRemoveEntryA() {

		MinPriorityQueue<Integer, Integer> mh = getPQ();
		
		// Insert one item and remove it via it's Entry object
		PQEntry<Integer, Integer> entry = mh.insert(1, 2);
		boolean removeResult = mh.remove(entry);
		
		assertTrue(removeResult, 
			   	   "remove() return is incorrect");
		...
		assertNotNull(entry,
		         "returned returned entry is incorrect");

		assertNotNull(entry.getElement(),
		         "entry element is incorrect");
		
		assertEquals(1, 
				     entry.getElement(),
			         "remove() item's element is incorrect");
